...
def highlight_dork_in_line(line, dorks, case_sensitive):
    for dork in dorks:
        ...
        if case_sensitive:
            ...
            highlighted_dork = f"{red_start}\\g<0>{red_end}"
...
def main():
    ...
    parser.add_argument("--dorks", metavar="dorks", nargs='+', help="List of search terms (dorks) to find in the files")
    parser.add_argument("--dork-file", metavar="dork_file", type=str, help="Path to a text file containing a list of dorks")
    ...
    if args.dork_file:
        with open(args.dork_file, 'r') as file:
            dorks = [line.strip() for line in file.readlines()]
    else:
        dorks = args.dorks

    if not dorks:
        print("No dorks provided. Use --dorks or --dork-file.")
        return

    file_types = ['.' + ft.lower() if not ft.startswith('.') else ft.lower() for ft in args.file_types] if args.file_types else None

    traverse_repo(args.directory_path, dorks, args.max_threads, file_types, args.exact_word, args.context, args.case_sensitive)
