...
js_files_dir = 'js-files'  # Replace with your directory path

# Function to search each file
def search_files(directory, pattern):
    regex = re.compile(pattern)
    for filename in os.listdir(directory):
        if filename.endswith(".js"):  # Checks if the file is a JavaScript file
            filepath = os.path.join(directory, filename)
            with open(filepath, 'r', encoding='utf-8', errors='ignore') as file:
                for line in file:
                    for match in regex.finditer(line):
                        print(f"Found in {filename}: {match.group()}")

# Run the search
search_files(js_files_dir, pattern)
